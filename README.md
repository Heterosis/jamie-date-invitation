...
| `telegram` | Public username without `@`, 1–32 letters, digits, or underscores | No direct recipient | Button becomes `SHARE ON TELEGRAM` and opens Telegram's chat picker with a draft. |
...
On `main`, `.github/workflows/pages.yml` runs unit tests, local browser tests, a production repository-base build, and deploys the `dist` artifact to the `github-pages` environment.
